class_name AirSlashAState
extends State

@onready var sword: Sword = $"../../Mesh/Skeleton3D/BoneAttachment3D/Sword"

const GRAVITY_STALL_DURATION := 0.18   # how long gravity is suppressed
const GRAVITY_SCALE_AFTER := 0.4       # reduced gravity after stall, before recovery
const HORIZONTAL_DRIFT := 0.6          # slight forward drift during swing
...
func enter() -> void:
	...
	# Stall vertical momentum — DMC style hang
...
func physics_process(delta: float) -> void:
	...
	if timer >= duration:
		if attack_buffered:
			state_machine.transition_to(state_machine.get_node("AirSlashBState"))
		else:
