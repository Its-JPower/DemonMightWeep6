...
@export var hit_invuln_time: float = 0.35   # i-frame window granted on landing a hit
func enter() -> void:
	player.play_hit_sfx()
...
func _on_hit(enemy: Enemy) -> void:
	...
	var damage := PlayerStats.sword_slash_b_damage
...
func _end_state() -> void:
	player.anim_player.play("Sword_Regular_B_Rec", -1, 1.0)
